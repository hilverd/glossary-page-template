...
The above assets are cached for 24 hours and get updated with any releases with a [major version number](https://semver.org/) of `5`. Alternatively you can change the URL to point to e.g. `@5.10` which gets all patch releases with version numbers of the form `5.10.x`. You can also use a specific version such as `5.10.1` — these are cached forever.
