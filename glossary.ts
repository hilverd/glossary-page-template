...
const containerElement: HTMLElement | null = document.getElementById(
...
if (containerElement) {
    ...
    const userEmailAddress: string | null =
        bodyDataset.userEmailAddress || null;

    const containerDataset: DOMStringMap = containerElement.dataset;

    const enableHelpForMakingChanges: boolean =
        containerDataset.enableHelpForMakingChanges === "true";
    const enableSavingChangesInMemory: boolean =
        containerDataset.enableSavingChangesInMemory === "true";
    const enableExportMenu: boolean =
    ...
    const enableLastUpdatedDates: boolean =
        containerDataset.enableLastUpdatedDates === "true";
    const versionNumber: number | null =
        Number(containerDataset.versionNumber) || null;
    const separateBackendBaseUrl: string | null =
        bodyDataset.separateBackendBaseUrl || null;
    const editorIsRunning: boolean =
        containerDataset.editorIsRunning === "true";

    const titleElement: HTMLElement | null = document.getElementById(
        "glossary-page-title",
    );
    const aboutElement: HTMLElement | null = document.getElementById(
        "glossary-page-about",
    );

    const glossaryElement: HTMLElement | null = document.getElementById(
        "glossary-page-items",
    );

    const aboutParagraph: string =
        aboutElement?.querySelector("p")?.textContent?.trim() || "";

    const aboutUlElement: HTMLElement | null =
        aboutElement?.querySelector("ul") || null;
    const aboutLiElements: HTMLElement[] = Array.prototype.slice.apply(
        aboutUlElement?.querySelectorAll("li") || [],
    );
    const aboutLinks: { href: string; body: string }[] = aboutLiElements.map(
        aboutLinkFromLiElement,
    );

    const tagDivElements: HTMLElement[] = Array.prototype.slice.apply(
        document.querySelectorAll("#glossary-page-tags > dl > div"),
    );

    const tagsWithDescriptions: { tag: string; description: string }[] =
        tagDivElements.map((tagDivElement) => {
            return {
                id: tagDivElement.dataset.id || self.crypto.randomUUID(),
                tag:
                    tagDivElement.querySelector("dt")?.textContent?.trim() ||
                    "",
                description:
                    tagDivElement.querySelector("dd")?.textContent?.trim() ||
                    "",
            };
        });

    const dlElement: HTMLElement | null =
        glossaryElement?.querySelector("dl") || null;
    const glossaryItemDivElements: HTMLElement[] = Array.prototype.slice.apply(
        dlElement?.querySelectorAll("div") || [],
    );
    const glossaryItems: any[] = glossaryItemDivElements.map(
        glossaryItemFromDivElement,
    );

    function aboutLinkFromLiElement(aboutLiElement: HTMLElement): {
        href: string;
        body: string;
    } {
        const aElement: HTMLAnchorElement | null =
            aboutLiElement.querySelector("a");
        return {
    ...
    function tagInItemFromButtonElement(
        buttonElement: HTMLButtonElement,
    ): string | undefined {
        return buttonElement?.textContent?.trim();
    }

    function glossaryItemFromDivElement(
        glossaryItemDivElement: HTMLElement,
    ): any {
        const id: string | undefined = glossaryItemDivElement.dataset.id;

        const dtElements: HTMLElement[] = Array.prototype.slice.apply(
            glossaryItemDivElement.querySelectorAll("dt"),
        );
        ...
        const ddElements: HTMLElement[] = Array.prototype.slice.apply(
            glossaryItemDivElement.querySelectorAll("dd"),
        );

        const definitionDdElements: HTMLElement[] = ddElements.filter(
            (ddElement) =>
        ...
        */
        const definition: string =
            definitionDdElements
                .map((ddElement) => ddElement.textContent?.trim())
                .join("\n\n") || "";

        const tagsDdElement: HTMLElement | undefined = ddElements.filter(
            (ddElement) => ddElement.className === "tags",
        )[0];
        const tagElements: HTMLButtonElement[] = Array.prototype.slice.apply(
            tagsDdElement?.querySelectorAll("button") || [],
        );
        const tags: (string | undefined)[] = tagElements.map((tagElement) =>
            tagInItemFromButtonElement(tagElement),
        );
        const hasDisambiguationTag: boolean =
            preferredTermDtElement?.querySelector("dfn span.disambiguation") !==
            null;

        const needsUpdatingDdElements: HTMLElement[] = ddElements.filter(
            (ddElement) => ddElement.className === "needs-updating",
        );
        const relatedTermDdElements: HTMLElement[] = ddElements.filter(
            (ddElement) => ddElement.className === "related-terms",
        );
        const relatedTerms: any[] =
            relatedTermDdElements.length > 0
                ? glossaryItemRelatedTermFromDdElement(relatedTermDdElements[0])
                : [];
        const lastUpdatedDate: string | undefined =
            glossaryItemDivElement.dataset.lastUpdated;
        const lastUpdatedByName: string | undefined =
            glossaryItemDivElement.dataset.lastUpdatedByName;
        const lastUpdatedByEmailAddress: string | undefined =
            glossaryItemDivElement.dataset.lastUpdatedByEmailAddress;

        return {
            id: id || self.crypto.randomUUID(),
            preferredTerm: glossaryItemTermFromDtElement(
                preferredTermDtElement,
            ),
            alternativeTerms: alternativeTermsDtElements.map(
                glossaryItemTermFromDtElement,
            ),
    ...
    function glossaryItemTermFromDtElement(
        dtElement: HTMLElement | undefined,
    ): {
        isAbbreviation: boolean;
        body: string;
    } {
        const dfnElement: HTMLElement | null =
            dtElement?.querySelector("dfn") || null;
        const isAbbreviation: boolean = Boolean(
            dfnElement?.querySelector("abbr"),
        );
        const dfnElementWithoutDisambiguationTag: HTMLElement | null =
            dfnElement?.querySelector("span") || dfnElement;
        const body: string =
            dfnElementWithoutDisambiguationTag?.textContent || "";
    ...
    function glossaryItemRelatedTermFromDdElement(
        ddElement: HTMLElement,
    ): any[] {
        return Array.prototype.slice
            .apply(ddElement.querySelectorAll("a"))
            .map((aElement) => {
                return {
    ...
    function themeValueForApp(defaultTheme: string): string {
        ...
        if (
            ...
            return inStorage; // treat themeInLocalStorage as undefined
        else return defaultTheme === "dark" ? "dark" : "system";
    }

    function useDarkMode(themeValueForApp: string): boolean {
        if (themeValueForApp === "system")
            return window.matchMedia("(prefers-color-scheme: dark)").matches;
        else return themeValueForApp === "dark";
    }
    ...
    const app = Elm.ApplicationShell.init({
        flags: {
            ...
            usingIncludedBackend:
                separateBackendBaseUrl != null || editorIsRunning,
    ...
    function allowBackgroundScrolling() {
        document
            .querySelector("body")
            ?.classList.toggle("overflow-hidden", false);
    }
    ...
    app.ports.preventBackgroundScrolling.subscribe(() => {
        document
            .querySelector("body")
            ?.classList.toggle("overflow-hidden", true);
    });
    ...
    app.ports.giveSearchFieldFocusOnceItIsPresent.subscribe(
        (elementId: string) => {
    ...
    app.ports.getCurrentDateTimeAndNewIdForSaving.subscribe(() => {
        app.ports.receiveCurrentDateTimeAndNewIdForSaving.send([
    ...
    domReady(() => {
        ...
        customElements.define(
            ...
            class extends HTMLElement {
                ...
                setTextContent() {
                    const datetimeAttribute: string | null =
        ...
        document.addEventListener("scroll", (event) => {
            ...
            const farAwayFromTheTop =
                document.documentElement.clientHeight * 2 < newScrollPosition;
